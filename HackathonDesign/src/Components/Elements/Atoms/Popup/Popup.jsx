...
const Popup = ({ truthValue, link, onClose }) => {
  ...
  const getBackgroundColor = (number) => {
    if (number < 30) return "#F1212F";
    if (number < 50) return "#F17821";
    if (number < 70) return "#214FF1";
    if (number < 90) return "#F1DC21";
    return "#0ACF83";
  };

  const backgroundImage = randomNewsNumber < 50 ? `url(${backgroundColorRed})` : `url(${backgroundColorGreen})`;
  ...
  return (
    <div id="PopupContainer" style={{ backgroundImage, height: "650px", width: "1000px", border: "1px solid red" }}>
